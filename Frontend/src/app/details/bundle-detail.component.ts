...
export class BundleDetailComponent implements OnInit {
  ...
  // ✅ Demo Bundles (später kannst du es auch aus JSON/Backend holen)
  ...
  addToCart(): void {
    ...
    const id = Number(this.bundle.id);
    const name = String(this.bundle.name ?? '');
    const price = Number(this.bundle.price ?? 0);
    const image = String(this.bundle.image ?? '');
    const weight = this.bundle.weight ? String(this.bundle.weight) : undefined;

    // CartService erwartet das gleiche Shape wie Produkt
    this.cart.add({ id, type: 'bundle', name, price, image, weight }, this.qty);
  }
